...
import { Settings, Plus, Send, RefreshCw, Trash2, CheckCircle2, XCircle, Clock } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid grid-cols-4 w-[600px]">
          ...
          <TabsTrigger value="campaigns">Broadcast Campaigns</TabsTrigger>
              ...
              <Card key={t.id} className={t.isActive ? "border-apollo-blue" : "opacity-75"}>
                ...
                <CardContent className="space-y-3 pb-3">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <Badge variant={t.status === "approved" ? "outline" : "destructive"} className="text-[10px] py-0">
                      {t.status}
                    </Badge>
                  </div>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleCreateCampaign} className="space-y-4 mt-2">
                  ...
                  <Button className="w-full mt-2" type="submit" disabled={createCampaign.isPending}>
                    {createCampaign.isPending ? "Queueing..." : "Launch Campaign Now"}
                  </Button>
                </form>
              </DialogContent>
            </Dialog>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="p-3 text-right">Launched</th>
                </tr>
              </thead>
              <tbody>
                {campaigns?.map((c) => {
                  const readRate = c.totalRecipients > 0 ? Math.round((c.readCount / c.totalRecipients) * 100) : 0;
                  return (
                    <tr key={c.id} className="border-b hover:bg-gray-50">
                      <td className="p-3 font-semibold">{c.name}</td>
                      <td className="p-3 font-mono text-xs">{c.segmentType}</td>
                      <td className="p-3">{c.totalRecipients}</td>
                      <td className="p-3 text-green-600 font-semibold">{c.deliveredCount}</td>
                      <td className="p-3">{readRate}%</td>
                      <td className="p-3">
