...
interface FeatureFlag {
  ...
  enabled: boolean;
  status: "disabled" | "preview" | "enabled";
  rolloutPercentage: number;
}

interface FeatureFlagsPanelProps {
  flags: FeatureFlag[] | undefined;
  onToggle: (id: string, status: "disabled" | "preview" | "enabled") => void;
  loading: boolean;
  error?: any;
}
...
export default function FeatureFlagsPanel({ flags, onToggle, loading, error }: FeatureFlagsPanelProps) {
  ...
  const renderFlag = (flag: FeatureFlag) => {
    const status = flag.status || (flag.enabled ? "enabled" : "disabled");
    return (
      <div key={flag.id} className="flex items-center justify-between p-4 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700/80 transition-all shadow-md animate-none">
        <div className="space-y-1 pr-4">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge className={`text-[10px] font-mono px-1.5 py-0 ${
              status === "enabled" ? "bg-emerald-950/40 text-emerald-400 border border-emerald-500/20" :
              status === "preview" ? "bg-amber-950/40 text-amber-400 border border-amber-500/20" :
        ...
        <select
          value={status}
          onChange={(e) => onToggle(flag.id, e.target.value as "disabled" | "preview" | "enabled")}
          className="bg-slate-950 border border-slate-800 rounded-lg text-slate-200 text-xs px-2.5 py-1.5 focus:outline-none focus:border-emerald-500/50 hover:border-slate-700 cursor-pointer font-mono"
        >
